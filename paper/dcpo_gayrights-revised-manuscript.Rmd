...
Of course, there has been an effort to employ latent-variable estimates recently.
...
After one author initially selected and coded the relevant questions, the other author reviewed them to confirm agreement on their inclusion in the raw data. 
In cases where there were differing opinions, the authors discussed the discrepancies as a group and updated the question selection accordingly.
Ultimately, we incorporated only those questions for which consensus was reached after discussion.
...
In practice, once we identified the survey items and their question scales, we used DCPOtools [@Solt2019] to automatically calculate the number of responses in each country-year expressing support for gay rights at or above a response category higher than the least supportive option.
This number is modeled on the expected probability that a random individual in a specific country-year gives a response at least as supportive as the given category for a question.
...
Ultimately, this approach allowed us to estimate the public’s latent attitude of support for gay rights.
More detailed technique explanation can be found on online appendix C. 
...
This article tries to overcome previous limitations concerning data about public opinion toward gay rights by presenting the SGR dataset. 
