...
On the other hand, we do have in the source data `r n_cy` country-years for which we have at least _some_ information about the extent of the tolerance of homosexuality in the population, that is, some `r round(covered_share_of_spanned)`% of the `r spanned_cy` country-years spanned by the data we collected.
...
Observations for every year in each country surveyed would total `r total_cy`, and a complete set of country-year-items would include `r {n_countries * n_years * n_items} %>% scales::comma()` observations.
Viewed from this complete-data perspective, the available data can be seen to be very, very sparse.
More optimistically, we do have in the source data `r n_cy` country-years for which we have at least _some_ information about the extent of the tolerance of homosexuality in the population, that is, some `r round(covered_share_of_spanned)`% of the `r spanned_cy` country-years spanned by the data we collected.
...
Despite its influence, this article was flagged by @Adamczyk2019 [, 415] in a recent review of the literature as one for which "more research is needed to replicate and confirm [its] findings."
Indeed, one recent work finds no support at all for the hypothesized relationship between inequality and attitudes toward homosexuality [@Zhang2019, 515].
...
This broader scope---along with the additional WVS survey waves conducted in the time between the two pieces were written---yielded 88 countries and 214 country-year observations, yielding an increased mean number of years observed of 2.4 per country.
So although the different conclusion reached by @Zhang2019 may reflect the larger number of countries that study included [see @Zhang2019, 517], it may also result from the inclusion of non-democratic countries. 
...
The TOLH data allow us to revisit this hypothesis with many, many more observations from a broader sample of countries than either of these two previous works and also to assess whether tolerance in democratic countries is distinctively sensitive to income inequality.
Our democratic sample includes the 36 democratic countries of the OECD, each observed in 21 (Costa Rica) to 49 (the United States) consecutive years (mean: `r cy_summary %>% nth(4) %>% round(1)` years, median: `r cy_summary %>% nth(3)` years), a total of `r data_combo %>% filter(country %in% oecd_democracies) %>% nrow()` country-year observations.
...
Economic inequality is measured using the Gini index of disposable income inequality from the Standardized World Income Inequality Database [@Solt2020].
...
Marriage equality is measured dichotomously, coded one in country-years where same-sex marriage was legal and zero otherwise.
A series of country-level variables to identify a country's religious heritage are also dichotomous: countries are coded as having alternately a Catholic, Orthodox, Eastern, or Islamic heritage, with those with a Protestant heritage treated as the reference category.
