...
The TOL-H dataset combines a comprehensive collection of responses to national and cross-national surveys with recent developments in latent-variable modeling of public opinion to provide estimates of public attitudes toward homosexuality that are comparable across many countries and over many years.
...
The European Social Survey, which surveyed respondents in 39 countries in as many as ten years (mean: 6.6 years) over two decades, provides the most over-time data for the most countries, followed by the AmericasBarometer (34 countries, mean years: 5.4).
...
poh_plot + plot_annotation(caption = str_wrap("Notes: Citation counts as reported by the Web of Science on June 21, 2023.  See the online appendix for details on the contruction of this sample of prominent articles.", width = 114))
```


Figure\nobreakspace{}\ref{poh_plot} shows how the available data have influenced scholarship on the topic.
Using the Web of Science and Google Scholar, we assembled a sample of prominent published articles on public opinion toward homosexuality (see the online appendix for details).
...
That a disproportionate share of research on public opinion regarding homosexuality---and indeed more of the available surveys on which this research is based---focuses on Europe and North America [see @Adamczyk2019, 410] only compounds these issues.
...
In the next section, we describe the national and cross-national surveys and the latent-variable model we use to this end, along with the resulting TOL-H dataset.
...
To generate estimates of tolerance of homosexuality cross-nationally and longitudinally that are comparable across space and over time, we first assemble a comprehensive collection of survey questions on the topic.
The relevant surveys are sparse, providing no relevant data for many countries and years, and incomparable, employing many different survey items, but collectively they have often asked questions about homosexuality over the past half-century.
...
On the other hand, we do have in the source data `r n_cy` country-years for which there is at least _some_ information about the extent of tolerance of homosexuality in the population, that is, very nearly `r round(covered_share_of_spanned)`% of the `r spanned_cy` country-years spanned by the data we collected.
...
At the other end of the spectrum, `r n_data_poorest` countries---`r data_poorest`---have only the minimum three observations required to be included in the source dataset at all.
...
First, it incorporates the _difficulty_ of each question's responses, that is, the amount of tolerance of homosexuality that is indicated by a given response. 
...
For example, strongly agreeing with the statement "homosexual couples should be able to adopt children" likely expresses more tolerance than merely responding that same-sex relationships should be not be criminalized.
...
With data on `r res_c` countries for `r min(res_y)` to `r max(res_y)` years---mean of `r mean(res_y) %>% sprintf("%.1f", .)` years---the TOL-H dataset is literally off the charts, that is, it is beyond the bounds of Figure\nobreakspace{}\ref{proj_kt_plot}.
