...
It further underscores what is already evident in Figure\nobreakspace{}\ref{cs_mry}: the cross-regional scope of the TOLH dataset allows comparison of countries too often neglected in political science analyses [see @Wilson2021].
...
```{r extval1, fig.cap="Construct Validation: Correlations Between TOLH Scores and Acceptance of Homosexuality Survey Items \\label{ext_val1}", fig.height=4, fig.width=7.5}
...
The piece argues that postmaterialist theory [see, e.g., @Inglehart2005] implies that greater inequality should be expected to yield greater intolerance of homosexuality: if economic prosperity is what allows societies to leave such biases behind, then when a society's prosperity is not broadly shared, more tolerant attitudes will not be broadly shared either.^[
An alternate argument would be that greater inequality gives wealthier individuals both greater means and motive to promote religiosity among their fellow citizens [see, e.g., @Solt2011a; @Solt2014a], and more religiosity in turn works to decrease tolerance [see, e.g., @Adamczyk2009].  We leave distinguishing between these arguments to future research.]
Supporting this view, its analysis found that more economic inequality leads to more intolerance of homosexuality.
Despite this article's influence, its conclusion was flagged by @Adamczyk2019 [, 415] in a recent review of the literature as one for which "more research is needed to replicate and confirm these findings."
Indeed, one recent work has found no support at all for the hypothesized relationship between inequality and attitudes toward homosexuality [@Zhang2019, 515].
...
The TOLH data allows us to revisit this hypothesis with many, many more observations from a broader sample of countries than either of these two previous works and also to assess whether tolerance in democratic countries is distinctively sensitive to income inequality.
Our democratic sample includes the 36 democracies of the OECD,^[
That is, this sample excludes the OECD's two members with competitive authoritarian regimes, Hungary and Turkey.] each observed in 21 (Costa Rica) to 49 (the United States) consecutive years (mean: `r cy_summary %>% nth(4) %>% round(1)` years, median: `r cy_summary %>% nth(3)` years), a total of `r data_combo %>% filter(country %in% oecd_democracies) %>% nrow()` country-year observations.
...
That is, the TOLH provides a number of country-years observations for our sample of democracies that is some `r {data_combo %>% filter(country %in% oecd_democracies) %>% nrow()/63} %>% round()` times greater than that considered in @Andersen2008, and it gives us a number of country-years in our sample of all countries that is about `r {data_combo %>% nrow()/214} %>% round()` times greater than that in the sample employed in @Zhang2019.
...
Data on GDP per capita (in thousands of constant 2015 U.S. dollars) are provided by the World Bank's World Development Indicators [@WB2023].
...
```{r resplot, fig.cap="Predicting Acceptance of Homosexuality \\label{model}", fig.height=6, fig.width=7.5}
