...
To address these issues, this article presents the Tolerance of Homosexuality (TOL-H) dataset.
...
poh <- readxl::read_excel(here("data-raw", "wos_poh1.xls")) %>% 
  janitor::clean_names() %>% 
...
poh_plot + plot_annotation(caption = str_wrap("Notes: Citation counts as reported by the Web of Science on June 21, 2023.  See Appendix A for details on the contruction of this sample of prominent articles.", width = 114))
```


Figure\nobreakspace{}\ref{poh_plot} shows how the available data have influenced scholarship on the topic.
Using the Web of Science and Google Scholar, we assembled a sample of prominent published articles on public opinion toward homosexuality (see Appendix A for details).
These articles had publication dates as early as `r summary(poh$year)[[1]]` and as late as `r summary(poh$year)[[6]]` (median: `r summary(poh$year)[[3]]`) and had Web of Science citation counts from `r summary(poh$hits)[[1]]` to ``r summary(poh$hits)[[6]]` times (median: `r summary(poh$hits)[[3]]`).
We then examined these articles to find the number of countries and years investigated in each.
As the zoomed portion of the plot highlights, many of these articles might be described as using case-study or small-n research designs.
...
@Clements2014, for example, tracks changes in British attitudes in 13 different survey projects conducted in 42 different years; @Yang1997 similarly reports trends in public opinion drawing on many surveys conducted in 26 different years in the United States.
Works considering data from more than five countries encompass less than a sixth of these articles.
One recent piece, @Hildebrandt2019, draws on the fifth and sixth waves of the WVS to create a single cross-section of 73 countries and argues that modernization leads to more tolerance and in turn, in democracies, more rights recognized; the influential @Adamczyk2009 similarly employs a single cross-section of the WVS, in its case the fourth wave, to examine how individuals' religiosity and their countries' religious heritages shape their attitudes about homosexuality.
...
The article examining the most country-years of public opinion in this sample is @Redman2018, an investigation of policy feedback on public opinion that uses the second through sixth WVS waves to provide a total of 203 country-years in 70 countries, that is, a number of mean observed years per country of 2.9.
...
Within this sample of prominent works, none of the articles including five or more countries are able to examine public opinion in more than five years in any of the countries included in their analyses.

That these L-shaped distributions in the available datasets and the resulting analyses should limit our confidence in our conclusions is readily evident.
...
The cross-national analyses on the horizontal, with just one or a few time points to leverage, on the other hand, often raise concerns that differences across countries are being conflated with over-time causal processes [see, e.g., @Jackman1985, 173-174].
...
In addition, previous studies have illustrated that features such as economic development [@Hooghe2013], historical legacy [@asal2013original], political context [@Zhang2019], and religious context [@van2013disapproval] have shape opinion toward homosexuality.
<!-- ^[ -->
...
\end{center}
```
# Appendix A: Sample of Published Articles on Public Opinion and Homosexuality

To generate a sample of published articles on public opinion and homosexuality, we combined results from two searches, the first on the Web of Science and the second on Google Scholar.
...
The fifty most cited research articles returned, excluding those without an empirical component, were retained for our sample.
...
To ensure comparability of citation counts, we collected the Web of Science records for this second group of articles.
Then we added the records for these Google Scholar results to our Web of Science sample and dropped duplicates.
Each of the articles was then consulted to identify the number of countries and mean years observed per country.
...
# Appendix B: Survey Items Used to Estimate Tolerance of Homosexuality
...
\noindent Table A2: Indicators Used in the Tolerance of Homosexuality Latent Variable Model
